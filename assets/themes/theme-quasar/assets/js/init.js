...
jQuery(document).ready(function () {
  ...
  //jQuery("#Home").localScroll(6000);
  jQuery("#navbar-brand").parallax("50%", 0.1);
  jQuery("#Home").parallax("50%", 0.1);
  jQuery("#Caracteristicas").parallax("50%", 0.3);
  jQuery("#Lugares").parallax("50%", 0.1);
  jQuery("#Opiniones").parallax("50%", 0.3);
  jQuery("#Equipo").parallax("50%", 0.1);
  jQuery("#Precios").parallax("50%", 0.1);
  jQuery("#Contactos").parallax("50%", 0.1);
...
jQuery(".navbar .nav > li > a").click(function () {
  jQuery(".navbar-collapse.navbar-ex1-collapse.in")
    .removeClass("in")
    .addClass("collapse")
    .css("height", "0");
});
...
$(document).ready(function () {
  $("#Send").click(function () {
    ...
    if (nameVal == "") {
      $("#name_error").html("");
      $("#name").after(
        '<label class="error" id="name_error">Please enter your name.</label>'
      );
      return false;
    } else {
      $("#name_error").html("");
    ...
    if (emailaddressVal == "") {
      $("#email_error").html("");
      $("#email").after(
        '<label class="error" id="email_error">Please enter your email address.</label>'
      );
      return false;
    } else if (!emailReg.test(emailaddressVal)) {
      $("#email_error").html("");
      $("#email").after(
        '<label class="error" id="email_error">Enter a valid email address.</label>'
      );
      return false;
    } else {
      $("#email_error").html("");
    ...
    if (nameVal == "") {
      $("#message_error").html("");
      $("#message").after(
        '<label class="error" id="name_error">Please enter your message.</label>'
      );
      return false;
    } else {
      $("#message_error").html("");
    ...
    $.post("post.php?" + $("#MYFORM").serialize(), {}, function (response) {
      if (response == 1) {
        $("#after_submit").html("");
        $("#Send").after(
          '<br/><BR/><div class="alert alert-success success" id="after_submit"><button type="button" class="close" data-dismiss="alert">x</button>Thank you! Your message has been sent.</div>'
        );
        change_captcha();
        clear_form();
      } else {
        $("#after_submit").html("");
        $("#Send").after(
          '<label class="error" id="after_submit">Wrong captcha! Refresh and try again.</label>'
        );
      }
    });
    return false;
  ...
  function change_captcha() {
    document.getElementById("captcha").src =
      "contact/get_captcha.php?rnd=" + Math.random();
  }
  function clear_form() {
    $("#name").val("");
    $("#email").val("");
